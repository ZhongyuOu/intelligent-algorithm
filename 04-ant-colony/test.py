...
# 构建“蚁群算法”的主体
class ACO(object):
    def __init__(self, ant_num=50, maxIter=10, alpha=1, beta=5, rho=0.1, Q=1):
    ...
    def deal_data(self, filename):
        ...
        self.cities = list(
            City(float(item[0]), float(item[1])) for item in temp_list
        )  # 2. 构建城市列表
        ...
        self.eta_mat = 1 / (
            self.city_dist_mat + np.diag([np.inf] * self.cities_num)
        )  # 5. 初始化启发函数矩阵
    ...
    def solve(self):
        ...
        while iterNum < self.maxIter:
            ...
            delta_phero_mat = np.zeros(
                (self.cities_num, self.cities_num)
            )  # 初始化每次迭代后信息素矩阵的增量
            ##########################################################################
